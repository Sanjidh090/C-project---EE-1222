//this file is bit updated version fo smartbuy.c beacuse here we no longer ask how many products you wanna buy.we just let you to enter 0 if you wanna close the deal//
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <stdbool.h>
...
int main() {
    ...
    while (1) {
        ...
        case 1: {
            ...
            while (true) {
                f = (int*)realloc(f, (numItems + 1) * sizeof(int));
                q = (int*)realloc(q, (numItems + 1) * sizeof(int));
                if (f == NULL || q == NULL) {
                ...
                printf("Enter the item no. (or 0 to finish): ");
                if (scanf("%d", &f[numItems]) != 1 || f[numItems] < 0 || f[numItems] > NUM_ITEMS) {
                    fprintf(stderr, "Error: invalid item number.\n");
                    while (getchar() != '\n'); // Clear the input buffer
                    continue;
                }
                if (f[numItems] == 0) break;

                printf("Enter the quantity: ");
                if (scanf("%d", &q[numItems]) != 1 || q[numItems] <= 0) {
                    fprintf(stderr, "Error: invalid quantity.\n");
                    while (getchar() != '\n'); // Clear the input buffer
                    continue;
                }
                if (q[numItems] > inStock[f[numItems] - 1]) {
                    fprintf(stderr, "Error: insufficient stock for %s.\n", foodName[f[numItems] - 1]);
                    continue;
                }

                total += price[f[numItems] - 1] * q[numItems];
                inStock[f[numItems] - 1] -= q[numItems];
                printf("You have ordered %d %s. Total cost so far: %.2lf\n", q[numItems], foodName[f[numItems] - 1], total);
